...
export class Modify1588411681523 implements MigrationInterface {
    name = 'Modify1588411681523'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("DROP INDEX `IDX_121aedb761fd50ef3b93de6616` ON `post_item`", undefined);
        await queryRunner.query("ALTER TABLE `post_item_image` ADD `modifiedAt` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)", undefined);
        await queryRunner.query("ALTER TABLE `post_item` ADD `htmlContent` mediumtext NULL", undefined);
        ...
        await queryRunner.query("ALTER TABLE `post_item` DROP COLUMN `contentPreview`", undefined);
        await queryRunner.query("ALTER TABLE `post_item` ADD `contentPreview` varchar(256) NULL", undefined);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query("ALTER TABLE `post_item` DROP COLUMN `contentPreview`", undefined);
        await queryRunner.query("ALTER TABLE `post_item` ADD `contentPreview` varchar(128) NULL DEFAULT 'NULL'", undefined);
        await queryRunner.query("ALTER TABLE `post_item` ADD CONSTRAINT `FK_583829a77a7b675981de33ecae2` FOREIGN KEY (`categoryId`) REFERENCES `category`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION", undefined);
        ...
        await queryRunner.query("ALTER TABLE `admin_session` ADD CONSTRAINT `FK_b41305caf8ef2f3847957746a2f` FOREIGN KEY (`userId`) REFERENCES `admin`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION", undefined);
        await queryRunner.query("ALTER TABLE `post_item` DROP COLUMN `htmlContent`", undefined);
        await queryRunner.query("ALTER TABLE `post_item_image` DROP COLUMN `modifiedAt`", undefined);
